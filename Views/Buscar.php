...
$palabra = isset($_GET['q']) ? trim($_GET['q']) : '';
$productos = $palabra !== '' ? $controller->buscarPorPalabra($palabra) : [];

$tituloPagina = 'Vokter - Resultados para "' . $palabra . '"';
...
<style>
  ...
  .no-results { padding: 60px 48px; text-align: center; color: #666; font-size: 1.1rem; }
</style>

<h1 class="page-heading">Resultados para "<?php echo htmlspecialchars($palabra); ?>"</h1>

<?php if (empty($productos)): ?>
  <p class="no-results">No encontramos productos que coincidan con tu búsqueda.</p>
<?php else: ?>
  <div class="products">
